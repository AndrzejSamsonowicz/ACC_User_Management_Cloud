    ...
    function createFoldersModal() {
        ...
        const styles = `
            <style>
                ...
                .treemap-radio-label input[type="radio"] {
                    cursor: pointer;
                    width: 16px;
                    height: 16px;
                    accent-color: #6f42c1;
                ...
                .treemap-close:hover { color: #ff6b00; transform: scale(1.2); }

                .treemap-filter-group {
                    display: flex;
                    gap: 12px;
                    align-items: center;
                    margin-left: 16px;
                ...
                .treemap-filter-radio input[type="radio"] {
                    cursor: pointer;
                    width: 14px;
                    height: 14px;
                    accent-color: #6f42c1;
                ...
                .treemap-loading-spinner {
                    ...
                    border-radius: 50%;
                    animation: treemapSpin 0.8s linear infinite;
                }
                @keyframes treemapSpin {
                    to { transform: rotate(360deg); }
                }
                @keyframes treemapPulse {
        ...
        svg.selectAll('rect.leaf')
            ...
            .on('mousemove', function(event, d) {
                ...
                if (treemapMode === 'folders') {
                    if (st === 'EMPTY') {
                        html = `<strong>${d.data.name}</strong><br><em>No users assigned</em><br>Path: ${folderPath}`;
                    } else {
                        html = `<strong>${d.data.name}</strong><br>Type: ${st}`;
                        if (st === 'USER' && d.data.email) html += ` — ${d.data.email}`;
                        if (st === 'COMPANY' && d.data.companyName) html += ` — ${d.data.companyName}`;
                        if (st === 'ROLE' && d.data.roleName) html += ` — ${d.data.roleName}`;
                        html += `<br>Folder: ${folderPath}`;
                    }
                } else {
                    html = `<strong>${d.data.name || d.data.path}</strong><br>User: ${d.parent?.data?.name || ''}`;
                }
