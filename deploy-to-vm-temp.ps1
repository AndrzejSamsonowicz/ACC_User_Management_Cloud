...
try {
    ...
    Write-Host "âœ— gcloud is not installed!" -ForegroundColor Red
...
if ($vmStatus -eq "RUNNING") {
    ...
    Write-Host "âœ“ VM started. Waiting 30 seconds for boot..." -ForegroundColor Green
    Start-Sleep -Seconds 30
} else {
    Write-Host "âœ— VM status unknown: $vmStatus" -ForegroundColor Red
    exit 1
}
